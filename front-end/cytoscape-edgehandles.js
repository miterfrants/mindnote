...
function PopupCenter(url, title, desc, w, h) {
...
var top = (height - h) / 2 / systemZoom + dualScreenTop
  ...
  var template= "<h1>{title}</h1><p>{desc}</p>"
  var result = template.replace(/{title}/gi, title)
...
function addCytoscapeListeners() {
  ...
  this.addListener(cy, 'click', 'node', function(e){
    if(!e.target._private.data.desc){
      return;
    }
    var desc = e.target._private.data.desc.toString().replace(/\n/gi,'<br/>').replace(/\r/gi,'<br/>');
    var title = e.target._private.data.name;
    
    PopupCenter("",title, desc, 500, 300);
  })
