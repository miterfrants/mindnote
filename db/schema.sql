...
ALTER TABLE public.board
    ...
    ADD GENERATED BY
        DEFAULT AS IDENTITY (SEQUENCE NAME
            public.board_id_seq START WITH 1 INCREMENT BY 1
            NO MINVALUE
            NO MAXVALUE
            CACHE 1);
...
ALTER TABLE public.image
    ...
    ADD GENERATED BY
        DEFAULT AS IDENTITY (SEQUENCE NAME
            public.image_id_seq START WITH 1 INCREMENT BY 1
            NO MINVALUE
            NO MAXVALUE
            CACHE 1);
...
ALTER TABLE public.node
    ...
    ADD GENERATED BY
        DEFAULT AS IDENTITY (SEQUENCE NAME
            public.node_id_seq START WITH 1 INCREMENT BY 1
            NO MINVALUE
            NO MAXVALUE
            CACHE 1);
...
ALTER TABLE public.node_relationship
    ...
    ADD GENERATED BY
        DEFAULT AS IDENTITY (SEQUENCE NAME
            public.node_relationship_id_seq START WITH 1 INCREMENT BY 1
            NO MINVALUE
            NO MAXVALUE
            CACHE 1);
...
ALTER TABLE public.transaction
    ...
    ADD GENERATED BY
        DEFAULT AS IDENTITY (SEQUENCE NAME
            public.transaction_id_seq START WITH 1 INCREMENT BY 1
            NO MINVALUE
            NO MAXVALUE
            CACHE 1);
...
ALTER TABLE public. "user"
    ...
    ADD GENERATED BY
        DEFAULT AS IDENTITY (SEQUENCE NAME
            public.user_id_seq START WITH 1 INCREMENT BY 1
            NO MINVALUE
            NO MAXVALUE
            CACHE 1);

--
-- Name: view_board; Type: VIEW; Schema: public; Owner: webservice
--

CREATE VIEW public.view_board AS
SELECT
    board.id,
    board.title,
    board.created_at,
    ...
    image.height,
    "user".username,
    CASE WHEN (latest_update_record.latest_updated_at IS NULL) THEN
        board.created_at
    WHEN (latest_update_record.latest_updated_at < board.updated_at) THEN
        board.updated_at
    ELSE
        latest_update_record.latest_updated_at
    END AS latest_updated_at
FROM (((public.board
        LEFT JOIN public.image ON (((board.image_id = image.id)
                    AND (image.owner_id = board.owner_id)
                    AND (image.deleted_at IS NULL))))
    LEFT JOIN public. "user" ON (("user".id = board.owner_id)))
    LEFT JOIN (
        SELECT
            max(node.updated_at) AS latest_updated_at,
            node.board_id
        FROM
            public.node
        GROUP BY
            node.board_id) latest_update_record ON ((latest_update_record.board_id = board.id)));

ALTER TABLE public.view_board OWNER TO webservice;
...
CREATE VIEW public.view_user AS WITH t AS (
    ...
    FROM
        public.transaction
    WHERE ((now() < (transaction.paid_at + '1 mon'::interval))
...
SELECT
    ...
    (
        SELECT
            CASE WHEN (sum(image.size) IS NULL) THEN
            (0)::numeric
        ELSE
            sum(image.size)
            END AS sum
        ...
        WHERE ((image.owner_id = "user".id)
            AND (image.deleted_at IS NULL))) AS storage_usage,
CASE WHEN (user_transaction.id IS NOT NULL) THEN
    TRUE
ELSE
    FALSE
END AS is_subscribed,
user_transaction.id AS transaction_id,
CASE WHEN (user_transaction.is_next_subscribe IS NOT NULL) THEN
...
--
-- Name: idx_image_owner; Type: INDEX; Schema: public; Owner: webservice
--

CREATE INDEX idx_image_owner ON public.image
