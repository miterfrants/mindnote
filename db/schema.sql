...
CREATE VIEW public.view_node_relationship AS
 SELECT node_relationship.parent_node_id,
  ...
  WHERE ((parent_node.deleted_at IS NULL) AND (child_node.deleted_at IS NULL) AND (node_relationship.deleted_at IS NULL));
...
CREATE UNIQUE INDEX relationship_key ON public.node_relationship USING btree (GREATEST(parent_node_id, child_node_id), LEAST(parent_node_id, child_node_id)) WHERE (deleted_at IS NULL);
